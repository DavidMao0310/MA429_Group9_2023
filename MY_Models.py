...
class Regression_models:
    ...
    def XGBR(self, searchCV=True, best_depth_input=5, best_Nestimators=200, best_L1_alpha=1,
             show_avsp=True, show_features_imp=False):
        if searchCV == True:
            # params = {'n_estimators': [100], 'max_depth': [12],
            #           'alpha': [0.2]}
            params = {'n_estimators': [50, 100, 150, 200], 'max_depth': [6, 8, 10, 12],
                      'alpha': [0.2, 0.5, 1, 2]}
            # Use GridSearchCV to search over the parameter grid
            grid_search = GridSearchCV(XGBRegressor(objective='reg:squarederror'), params, cv=5,
            ...
            best_parameters = {'n_estimators': best_Nestimators, 'max_depth': best_depth_input, 'alpha': best_L1_alpha}
    ...
    def NN_sequential(self, loss_func='rmse', epoch_num=30, bmc_noise_var=160, huber_delta=1):
        ...
        NN_model.add(layers.Dense(200, activation='relu', kernel_regularizer=regularizers.l2(0.01)))
        NN_model.add(layers.Dense(400, activation='relu', kernel_regularizer=regularizers.l2(0.01)))
        NN_model.add(layers.Dense(200, activation='relu', kernel_regularizer=regularizers.l2(0.01)))
        NN_model.add(layers.Dense(100, activation='relu', kernel_regularizer=regularizers.l2(0.01)))
    ...
    def label_outliers_zscore(self, col):
        ...
        return np.where(np.array([1 if z > 3 else 0 for z in z_scores]) == 0)[0]

    def do_auto_encoder(self, train_normal_data, encoding_dim):
        input_dim = self.x_train.shape[1]
        input_layer = layers.Input(shape=(input_dim,))
        encoder = layers.Dense(encoding_dim, activation="tanh",
        ...
        autoencoder.fit(train_normal_data, train_normal_data, epochs=50, batch_size=32, shuffle=True, verbose=1)
        ...
        self.y_train_aec = np.ravel(self.y_train_old.drop(train_outliers, axis=0))
    ...
    def autoencoder_with_xgboost(self, best_parameters_normal, best_parameters_abnormal, encoding_dim=64,
                                 searchCV_ab=False):
        helper_index = self.label_outliers_zscore(self.y_train_old[self.y_train_old.columns[0]])
        x_train_normal = self.x_train[helper_index]
        self.do_auto_encoder(x_train_normal, encoding_dim=encoding_dim)
        xgb_normal = XGBRegressor(eval_metric='rmse', n_estimators=best_parameters_normal['n_estimators'],
                                  max_depth=best_parameters_normal['max_depth'], alpha=best_parameters_normal['alpha'])
        ...
        train_info_df_normal = pd.DataFrame({'true': list(self.y_train_aec), 'pred': list(train_predictions)})
        test_info_df_normal = pd.DataFrame({'true': list(self.y_test_aec), 'pred': list(test_predictions)})
        if searchCV_ab == True:
        ...
        xgb_abnormal = XGBRegressor(eval_metric='rmse', n_estimators=best_parameters_abnormal['n_estimators'],
        ...
        train_info_df_abnormal = pd.DataFrame({'true': list(self.y_train_aec2), 'pred': list(train_predictions2)})
        test_info_df_abnormal = pd.DataFrame({'true': list(self.y_test_aec2), 'pred': list(test_predictions2)})
        all_train_info = pd.concat([train_info_df_normal, train_info_df_abnormal], axis=0)
        all_test_info = pd.concat([test_info_df_normal, test_info_df_abnormal], axis=0)
    ...
    def XGBoost_with_NN(self, searchCV=False, best_depth_input=10, best_Nestimators=200, best_L1_alpha=1,
                        show_avsp=True, show_features_imp=False, epochs=100):
        xgb_model = self.XGBR(searchCV=searchCV, best_depth_input=best_depth_input, best_Nestimators=best_Nestimators,
                              best_L1_alpha=best_L1_alpha,
                              show_avsp=show_avsp, show_features_imp=show_features_imp)
        ...
        NN_model.fit(self.x_train, train_residuals, epochs=epochs, batch_size=300, verbose=2)
